<template>
  <layout-content>
    <complex-table :header="$t('route.message')" @selection-change="handleSelectionChange" :data="data" :pagination-config="paginationConfig">
      <template #header>
        <el-button-group>
          <el-button size="small" :disabled="Selection.length < 1" round @click="markAsRead()">{{$t('message.mark_as_read')}}</el-button>
          <el-button size="small" :disabled="Selection.length < 1" round @click="remove()">{{$t('commons.button.delete')}}</el-button>
        </el-button-group>
      </template>
