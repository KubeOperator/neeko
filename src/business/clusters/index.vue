      ...
      <div v-if="newManifest.coreVars.length !== 0 && oldManifest.coreVars !== 0">
        <div align="center" style="margin-top: 15px">
          <table style="width: 90%" class="myTable">
            <thead>
              <tr>
                <td class="left">{{ $t("commons.table.name") }}</td>
                <td class="left">{{ $t("cluster.upgrade.current_version") }}</td>
                <td class="left">{{ $t("cluster.upgrade.upgrade_version") }}</td>
              </tr>
            </thead>
            <tbody>
              <tr>
...
import { getClusterStatus, initCluster, upgradeCluster, openLogger, importCluster, deleteCluster, healthCheck, clusterRecover, searchClusters } from "@/api/cluster"
