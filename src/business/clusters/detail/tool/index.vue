      ...
      <div v-loading="loading" v-for="tool in tools" :key="tool.name">
        <el-col :span="6">
          <el-card style="margin-left:10px; margin-top:10px; height: 180px" class="box-card">
            ...
            <el-button v-if="tool.higher_version && tool.status === 'Running'" @click="onUpgrade(tool)" size="small"
      ...
      <el-form label-position='left' :model="toolForm" ref="toolForm" label-width="140px">
        ...
        <div v-if="toolForm.name === 'chartmuseum'">
          ...
          <div v-if="toolForm.vars['persistence_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')" prop="vars.persistence_storageClass"
                          :rules="requiredRules">
            ...
            <el-form-item :label="$t('cluster.detail.tool.storage_size')" prop="vars.persistence_size"
                          :rules="numberRules">
              <el-input-number :step="1" step-strictly style="width: 80%" v-model="toolForm.vars['persistence_size']"
                               clearable></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select style="width: 80%" filterable v-model="toolForm.vars['nodeSelector_kubernetes\\_io/hostname']"
                         clearable>
        ...
        <div v-if="toolForm.name === 'prometheus'">
          <el-form-item :label="$t('cluster.detail.tool.data_retention')" prop="vars.server_retention"
                        :rules="numberRules">
            <el-input-number :step="1" step-strictly style="width: 80%" v-model="toolForm.vars['server_retention']"
                             clearable></el-input-number>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
          ...
          <div v-if="toolForm.vars['server_persistentVolume_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')" prop="vars.persistentVolume_storageClass"
                          :rules="requiredRules">
              <el-select style="width: 80%" filterable v-model="toolForm.vars['server.persistentVolume_storageClass']"
                         clearable>
                <el-option v-for="item of storages" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tool.storage_size')" prop="vars.persistentVolume_storageClass"
                          :rules="numberRules">
              <el-input-number :step="1" step-strictly style="width: 80%"
                               v-model="toolForm.vars['server_persistentVolume_size']" clearable></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select style="width: 80%" filterable
                         v-model="toolForm.vars['server_nodeSelector_kubernetes\\_io/hostname']" clearable>
                <el-option v-for="item of nodes" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
          </div>
        ...
        <div v-if="toolForm.name === 'logging'">
          <el-form-item :label="$t('cluster.detail.tool.search_index')"
                        prop="vars.fluentd__elasticsearch_elasticsearch_logstashPrefix" :rules="requiredRules">
            <el-input style="width: 80%" v-model="toolForm.vars['fluentd__elasticsearch_elasticsearch_logstashPrefix']"
                      clearable></el-input>
            <div><span class="input-help">{{$t('cluster.detail.tool.search_index_help')}}</span></div>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.replicas')" prop="vars.elasticsearch_replicas"
                        :rules="numberRules">
            <el-input-number :step="1" step-strictly style="width: 80%" @blur="checkReplicas"
                             v-model="toolForm.vars['elasticsearch_replicas']" clearable></el-input-number>
          ...
          <el-form-item :label="$t('cluster.detail.tool.hip_memery')" prop="vars.elasticsearch_esJavaOpts_item"
                        :rules="numberRules">
            <el-input-number :step="1" step-strictly style="width: 80%"
                             v-model="toolForm.vars['elasticsearch_esJavaOpts_item']" clearable></el-input-number>
          ...
          <div v-if="toolForm.vars['elasticsearch_persistence_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')"
                          prop="vars.elasticsearch_volumeClaimTemplate_storageClassName" :rules="requiredRules">
              <el-select style="width: 80%" filterable
                         v-model="toolForm.vars['elasticsearch_volumeClaimTemplate_storageClassName']" clearable>
                <el-option v-for="item of storages" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tool.storage_size')"
                          prop="vars.elasticsearch_volumeClaimTemplate_resources_requests_storage" :rules="numberRules">
              <el-input-number :step="1" step-strictly style="width: 80%"
                               v-model="toolForm.vars['elasticsearch_volumeClaimTemplate_resources_requests_storage']"
                               clearable></el-input-number>
            </el-form-item>
          </div>
        </div>
        ...
        <div v-if="toolForm.name === 'loki'">
          ...
          <div v-if="toolForm.vars['loki_persistence_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')" prop="vars.loki_persistence_storageClassName"
                          :rules="requiredRules">
              <el-select style="width: 80%" filterable v-model="toolForm.vars['loki_persistence_storageClassName']"
                         clearable>
                <el-option v-for="item of storages" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tool.storage_size')" prop="vars.loki_persistence_size"
                          :rules="numberRules">
              <el-input-number :step="1" step-strictly style="width: 80%"
                               v-model="toolForm.vars['loki_persistence_size']" clearable></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
              <el-select style="width: 80%" filterable
                         v-model="toolForm.vars['loki_nodeSelector_kubernetes\\_io/hostname']" clearable>
                <el-option v-for="item of nodes" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
          </div>
        ...
        <div v-if="toolForm.name === 'grafana'">
          <el-form-item :label="$t('cluster.detail.tool.password')" prop="vars.adminPassword" :rules="passwordRules">
            <el-input @blur="checkPassword" type="password" style="width: 80%" v-model="toolForm.vars['adminPassword']"
                      clearable></el-input>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.password_re')" prop="vars.adminPasswordRe"
                        :rules="passwordRules">
            <el-input @blur="checkPassword" type="password" style="width: 80%"
                      v-model="toolForm.vars['adminPasswordRe']" clearable></el-input>
            <div v-if="!isPasswordValid"><span
              class="input-error">{{$t('cluster.detail.tool.grafana_password_help')}}</span></div>
          </el-form-item>
          <el-form-item :label="$t('cluster.detail.tool.enable_storage')">
            <el-switch style="width: 80%" v-model="toolForm.vars['persistence_enabled']"></el-switch>
          </el-form-item>
          <div v-if="toolForm.vars['persistence_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')" prop="vars.persistence_storageClassName"
                          :rules="requiredRules">
              <el-select style="width: 80%" filterable v-model="toolForm.vars['persistence_storageClassName']"
                         clearable>
                <el-option v-for="item of storages" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
        ...
        <div v-if="toolForm.name === 'kubeapps'">
          ...
          <div v-if="toolForm.vars['postgresql_persistence_enabled']">
            <el-form-item :label="$t('cluster.detail.tool.storage_class')" prop="vars.global_storageClass"
                          :rules="requiredRules">
              <el-select style="width: 80%" filterable v-model="toolForm.vars['global_storageClass']" clearable>
                <el-option v-for="item of storages" :key="item" :value="item">{{item}}</el-option>
              </el-select>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tool.storage_size')" prop="vars.postgresql_persistence_size"
                          :rules="numberRules">
              <el-input-number :step="1" step-strictly style="width: 80%"
                               v-model="toolForm.vars['postgresql_persistence_size']" clearable></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('cluster.detail.tag.node')">
