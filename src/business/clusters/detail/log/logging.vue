    ...
    methods: {
      search(condition) {
        ...
        if(condition.match_info !== undefined) {
          queryArry.push({match: {message: {query: condition.match_info.value}}});
        }
        ...
        if(condition.namespace !== undefined) {
          if (condition.namespace.value.indexOf('-') === -1) {
            queryArry.push({term: {'kubernetes.namespace_name': this.namespace.value}});
          } else {
        ...
        if(condition.pod !== undefined) {
          if (condition.pod.value.indexOf('-') === -1) {
            queryArry.push({term: {'kubernetes.pod_name': condition.pod.value}});
          } else {
        ...
        if(condition.container !== undefined) {
          if (condition.container.value.indexOf('-') === -1) {
            queryArry.push({term: {'kubernetes.container_name': this.container.value}});
          } else {
