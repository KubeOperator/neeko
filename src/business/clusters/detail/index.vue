  ...
  <layout-content :header="$t('cluster.cluster_detail') + '  -  ' + clusterName" :back-to="{ name: 'ClusterList' }">
    <div v-loading="loading">
      <el-menu @select="search" router :default-active="$route.path" mode="horizontal">
        ...
        <el-menu-item v-if="source !== 'external'" :index="'/clusters/detail/'+project+'/'+name+'/backup'">{{$t('cluster.detail.backup.backup_recover')}}</el-menu-item>
        <el-menu-item v-if="source !== 'external'" :index="'/clusters/detail/'+project+'/'+name+'/component'">{{$t('cluster.detail.tag.component')}}</el-menu-item>
        ...
        <el-menu-item v-if="source !== 'external'" :index="'/clusters/detail/'+project+'/'+name+'/task'">{{$t('cluster.detail.tag.task')}}</el-menu-item>
      </el-menu>
      <br />
      <div>
