  ...
  <div>
    ...
    <el-drawer :title="$t('message.message_in_station')" :visible.sync="drawer" direction="rtl" size="25%" :modal="false">
      <div class="notice-list" v-infinite-scroll="next" v-loading="letterLoading">
        <div class="notice-item" v-for="(item, index) in messages" :key="index" @mouseover="hover = index" @mouseleave="hover = null" @click="open(item)">
          <div class="notice-title">
            <div class="text">
              <svg v-if="item.readStatus === 'READ'" class="icon svg-icon" aria-hidden="true" :style="{ color: '#969696'}" size="5x">
                <use xlink:href="#iconyidu1"></use>
              </svg>
              <svg v-if="item.readStatus === 'UNREAD'" class="icon svg-icon" aria-hidden="true" :style="{ color: '#0079b8' }">
