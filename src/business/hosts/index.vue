<template>
  <layout-content :header="$t('host.host')">
    <complex-table :data="data" :pagination-config="paginationConfig" @search="search" :selects.sync="hostSelections">
      <template #header>
        <el-button-group>
          ...
          <el-button :disabled="hostSelections.length<1" size="small" type="danger" @click="bactchDelete()">{{$t('commons.button.delete')}}</el-button>
        </el-button-group>
      </template>
