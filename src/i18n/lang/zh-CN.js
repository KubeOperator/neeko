...
const message = {
  commons: {
    ...
    validate: {
      ...
      ip_range_error: "IP 范围错误",
      cidr_error: "请输入正确的 cidr！",
      subnet_error: "子网错误",
  ...
  cluster: {
    ...
    detail: {
      ...
      component: {
        ...
        METRICS_SERVER_HELPER: "Metrics Server 是 Kubernetes 内置自动缩放管道的可扩展、高效的容器资源指标来源。",
        TRAEFIK_HELPER: "Traefik 是一种现代 HTTP 反向代理和负载均衡器，可以轻松部署微服务。",
        NGINX_HELPER: "ingress-nginx 是 Kubernetes 的入口控制器，使用 NGINX 作为反向代理和负载均衡器。",
        GPU_HELPER: "GPU Operator 允许 Kubernetes 集群的管理员像管理集群中的 CPU 节点一样管理 GPU 节点。",
        DNS_CACHE_HELPER: "NodeLocal DNSCache 通过在集群节点上作为 DaemonSet 运行 DNS 缓存代理来提高集群 DNS 性能。",
  ...
  setting: {
    ...
    kubepi_password_help: "admin 用户默认密码为 kubepi",
